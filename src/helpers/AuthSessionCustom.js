...
async function startAsync(options) {
    ...
    if (_authLock) {
        if (__DEV__) {
            console.warn('Attempted to call AuthSessionCustom.startAsync multiple times while already active. Only one AuthSessionCustom can be active at any given time.');
        }
        return { type: 'locked' };
    }
